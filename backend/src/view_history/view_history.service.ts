...
import { ViewHistoryResponseDto, RecentViewHistoryResponseDto, ViewHistoryItemDto } from './dtos/view-history.response.dto';
...
export class ViewHistoryService {
     ...
     */
    async deleteAllViewHistory(userId: number): Promise<{ message: string; deletedCount: number }> {
        // Verify user exists
        const user = await this.prismaService.users.findUnique({
            where: { id: userId }
        });
     ...
     */
    private mapViewHistoryToDto(viewHistory: any, dish: any): ViewHistoryResponseDto {
